...
var Input = (function () {
  function Input() {
    ...
    this.minVotes = 50;
    this.minAverage = 6.0;
    this.skipGenres = ['Foreign', 'TV Movie'], this.timestamp = Math.round(Date.now() / 1000);
  ...
  _createClass(Input, [{
    ...
    key: 'setCurrentGenres',
    value: function setCurrentGenres(genres) {
      var currentGenres = {};
    ...
    key: 'refreshData',
    value: function refreshData(http) {
    ...
    key: 'recommend',
    value: function recommend(http) {
      var parameters = this.convertSelection();
      var request = this.requestUrl + '/discover/movie?api_key=' + this.apiKey + parameters + '&vote_count.gte=' + this.minVotes + '&vote_average.gte=' + this.minAverage;
      var randomize = this.randomize;

      return http.get(request, function (data) {
        var page = randomize(data.total_pages);

        return http.get(request + '&page=' + page, function (data) {
          var result = randomize(data.results.length) - 1;
    ...
    _createClass(Orbs, [{
      ...
      value: function createStyle() {
        ...
        for (var i = 1; i <= this.amount; i++) {
          var randomLeftStart = input.randomize(this.reach + 1) - 1;
          var randomTopStart = input.randomize(this.reach + 1) - 1;
          var randomLeftEnd = input.randomize(this.reach + 1) - 1;
          var randomTopEnd = input.randomize(this.reach + 1) - 1;
      ...
      value: function createOrbs() {
        ...
        for (var i = 1; i <= this.amount; i++) {
          ...
          var baseDuration = Math.round(this.duration / 2);
          var duration = input.randomize(baseDuration);
          duration = baseDuration + duration;

          var randomSize = input.randomize(sizes.length) - 1;
  ...
  var app = new Vue({
    ...
    methods: {
      ...
      recommend: function recommend() {
        ...
        var recommend = Promise.resolve(input.recommend(this.$http));
        ...
        recommend.then(function (result) {
          ...
          for (var option in input.data.options) {
